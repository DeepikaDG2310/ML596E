...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Predict the app dangerousness usinig the BERT model finetuned for text classification."
    )
    parser.add_argument(
        "--data_file",
        type=str,
        required=True,
        help="Path to the CSV file containing texts.",
    )
    parser.add_argument(
        "--output_file",
        type=str,
...
def predict_reviews_from_file(
    file_path, tokenizer, model, max_length, batch_size, device
):
...
def main():
    ...
    df_with_predictions = predict_reviews_from_file(
        file_path, tokenizer, model, max_length, batch_size, device
    )
